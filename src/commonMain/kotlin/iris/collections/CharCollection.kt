...
interface CharCollection : PrimitiveCollection {
    ...
    fun sum(): Int {
        var s = 0
        val it = iterator()
        while (it.hasNext()) s += it.next().code
        return s
    }
    ...
    fun first(): Char {
        val it = iterator()
        if (!it.hasNext()) throw NoSuchElementException("Empty CharCollection")
        return it.next()
    }
    ...
    fun firstOrElse(default: Char): Char {
        val it = iterator()
        return if (it.hasNext()) it.next() else default
    }

    fun lastOrElse(default: Char): Char {
        val it = iterator()
    ...
    fun minOrElse(default: Char): Char {
        val it = iterator()
        if (!it.hasNext()) return default
        var m = it.next()
        while (it.hasNext()) {
    ...
    fun maxOrElse(default: Char): Char {
        val it = iterator()
        if (!it.hasNext()) return default
        var m = it.next()
        while (it.hasNext()) {
